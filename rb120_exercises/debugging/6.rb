...
# my answer was to call #to_s on each instance... but need to implement in the class!

# when we invoke Array#sort, it sort the elements based on comparisons done by the <=> method. The objects in the collection we are sorting must have access to a <=> method, but our Length class does not have one, thus we get an error message.

# we can fix this by implementing a Length#<=> method