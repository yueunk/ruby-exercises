...
def tf(term, document) # 3
  document.split(/[\s-]/).count { |word| word.downcase == term } # split the document into an array of words; count the word if it matches a term. the higher the count, the more important.
end
...
def idf(term, documents)
  number_of_documents = documents.length # how many documents
  number_of_documents_with_term = documents.count { |d| tf(term, d) > 0 } # count a document that includes the term at least once
  return 0 if number_of_documents_with_term == 0 # further
  Math.log(number_of_documents.to_f / number_of_documents_with_term.to_f) # logarithm of the given argument; default base is e (natural logarithm); logarithm is an inverse function to exponentiation
end
...
#further - remove document1 from the documents collection, and check the TF-IDF value of 'quantum' again.
# - Infinity is returned when the divisor is 0